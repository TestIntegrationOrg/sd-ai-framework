...
def _task(feature: str, *, token: str, subject: str = "requirement:FR-001") -> RemediationTask:
    return RemediationTask(
        ...
        subject=subject,
        summary="Implement the current requirement without changing specification truth.",
...
def _accepted_task(
    ...
    *,
    ...
    impl_result = _passed(implementation, root, "deterministic provider boundary: implementation passed")
    ...
    spec_result = _passed(spec, root, "specification compliance passed")
...
def _typed_evidence(
    ...
    *,
    ...
    record = TraceEvidence(
        ...
        bindings=(EvidenceBinding(EvidenceBindingKind.SOURCE, source, _digest(source_path)),),
...
def _critical_architecture(context: FeatureContext) -> None:
    artifacts = {
        ...
        "tasks.yaml": "tasks:\n  - id: TASK-1\n    title: Implement governed retry\n    traces_to: [FR-001, NFR-001, AC-001]\n",
    }
    for relative, content in artifacts.items():
        _write(context.artifact(relative), content)


def test_journey_a_small_feature_reaches_fresh_spec_driven_completion(tmp_path: Path) -> None:
    ...
    assert sdai_main(
        [
            ...
            FEATURE_A,
            "--title",
            "Small reference feature",
            "--description",
            "Exercise lightweight spec driven delivery",
            "--path",
            str(root),
        ]
    ) == 0
    ...
    task = _task(FEATURE_A, token="a107a107a107a107")
    ...
    chain = _accepted_task(root, ledger, task, dispatch=dispatch)

    report = evaluate_task_completion(root, ledger, task, attempt=1, risk="trivial")
    assert report.passed
    assert all(result.git_commit == _git(root, "rev-parse", "HEAD") for result in chain)
    completed = complete_isolated_task(root, ledger, task, attempt=1, risk="trivial")

    state = ledger.reconstruct()
    assert state.task_map()[task.task_id].status == "completed"
    assert completed.git_commit == _git(root, "rev-parse", "HEAD")
    contracts = {
...
def test_journey_b_critical_enterprise_feature_is_governed_without_identity_claims(
    tmp_path: Path,
) -> None:
    root = tmp_path / "journey-b"
    _git_project(root)
    _write(root / "specs" / FEATURE_B / "00-intake.md", "# Critical enterprise intake\n")
    context = FeatureContext(root, FEATURE_B)
    _critical_architecture(context)

    modern = root / "specs" / "changes" / FEATURE_B
    _write(
        modern / "requirements.md",
        "# Requirements\n\n- FR-001: Sign café requests. TASK-001 implements it and TEST-001 verifies it.\n- NFR-001: Signing preserves Δ behavior. TASK-001 implements it and TEST-001 verifies it.\n",
    )
    _write(modern / "architecture.md", "# Architecture\n\nThe signing component implements FR-001 and NFR-001.\n")
    _write(modern / "plan.md", "# Plan\n\nImplement TASK-001 for FR-001 and NFR-001, then run TEST-001.\n")
    _write(modern / "tasks.md", "# Tasks\n\n- [ ] TASK-001: Implement FR-001 and NFR-001; verified by TEST-001.\n")
    _write(modern / "tests.md", "# Tests\n\n- TEST-001: Verify FR-001 and NFR-001 through TASK-001.\n")
    ...
    org_policy = tmp_path / "journey-b-org-policy.yaml"
    _write(
        org_policy,
        "version: 1\nproviders: {}\ncapabilities: {}\nexecution: {}\nskills: {}\narchitecture_validation:\n  allow_waivers: false\n",
    )
    completion_policy = tmp_path / "journey-b-completion-policy.yaml"
    _write(
        completion_policy,
    ...
    _git(root, "add", ".")
    _git(root, "commit", "-m", "journey B governed specification and architecture")
    baseline = _git(root, "rev-parse", "HEAD")
    ledger = create_execution_run(root, FEATURE_B, "enterprise", baseline, run_id="journey-b")
    task = _task(FEATURE_B, token="b107b107b107b107")
    dispatch = prepare_implementation_dispatch(ledger, task)
    ...
    chain = _accepted_task(root, ledger, task, dispatch=dispatch)
    assert all(result.git_commit == head for result in chain)

    evidence = {
        "test": _typed_evidence(
            root,
            FEATURE_B,
            evidence_id="B-TEST",
            kind=EvidenceKind.TEST,
            subject=task.subject or f"task:{task.task_id}",
            source="src/signing.py",
            result={"suite": "reference", "passed": True},
        ),
        "quality": _typed_evidence(
            root,
            FEATURE_B,
            evidence_id="B-QUALITY",
            kind=EvidenceKind.QUALITY,
            subject=task.subject or f"task:{task.task_id}",
            source="src/signing.py",
            result={"qualityGate": "passed"},
        ),
        "security": _typed_evidence(
            root,
            FEATURE_B,
            evidence_id="B-SECURITY",
            kind=EvidenceKind.SECURITY,
            subject=task.subject or f"task:{task.task_id}",
            source="src/signing.py",
            result={"threatModelReviewed": True, "securityGate": "passed"},
        ),
    ...
    assert report.passed
    assert {item.dimension.value for item in report.findings} >= {
        "spec-review",
        "code-quality-review",
        "test",
        "quality",
    ...
    completed = complete_isolated_task(
        root,
        ledger,
        task,
        attempt=1,
    ...
    assert completed.payload["risk"] == "critical"
    ...
    for requirement in ("FR-001", "NFR-001"):
        _typed_evidence(
            root,
            FEATURE_B,
            evidence_id=f"B-EVIDENCE-{requirement}",
            kind=EvidenceKind.TEST,
        ...
        _semantic_review(
            root,
            FEATURE_B,
            review_id=f"B-REVIEW-{requirement}",
    ...
    _semantic_review(
        root,
        FEATURE_B,
        review_id="B-REVIEW-FAILURE",
        dimension=SemanticReviewDimension.FAILURE_BEHAVIOR,
        subject=f"feature:{FEATURE_B}",
        context_name="failure",
    )
    converged = run_convergence(root, FEATURE_B, risk="standard", max_rounds=3, environ={})
    assert converged.status is ConvergenceStatus.VERIFIED
    assert len(converged.rounds) == 2
    assert ledger.reconstruct().task_map()[task.task_id].status == "completed"


def _multi_repo_repository(path: Path, prefix: str) -> tuple[Path, str]:
...
def _write_pr_evidence(repository: Path, repository_id: str, prefix: str, implementation_commit: str) -> None:
    payload = {
...
def test_journey_c_multi_repo_source_truth_routes_executes_and_verifies_all_participants(
    tmp_path: Path,
    ...
    create_store(store, "reference-specs", "1.0.0", description="Central reference source of truth")
    ...
    implementation_commits: dict[str, str] = {}
    for repository_id, prefix in (("api", "API"), ("ui", "UI")):
        repository, commit = _multi_repo_repository(tmp_path / f"journey-c-{repository_id}", prefix)
        repositories[repository_id] = repository
        implementation_commits[repository_id] = commit
        _write_pr_evidence(repository, repository_id, prefix, commit)
    ...
    verification = verify_all_repositories(project, FEATURE_C, risk="standard", environ={})
...
def test_journey_d_interrupted_execution_resumes_without_duplicate_completed_work(tmp_path: Path) -> None:
    ...
    workflow_path = root / ".sdai" / "workflows" / "reference-long-running.yaml"
    _write(
        workflow_path,
        yaml.safe_dump(
            {
    ...
    before_events = ledger.load_events()
    completed_before = [event for event in before_events if event.kind == "task.completed"]
    assert len(completed_before) == 1
    completed_event = completed_before[0]
    completed_bindings = {
        binding.source: (root / binding.source).read_bytes()
        for binding in completed_event.bindings
        if (root / binding.source).is_file()
    }
    checkpoint_before = ledger.load_checkpoint()
    assert checkpoint_before is not None

    resumed_ledger = load_execution_run(root, FEATURE_D, "journey-d")
    result = execute_workflow_graph(resolution, resumed_ledger, leaf_executor=executor)

    assert result.status is WorkflowExecutionStatus.SUCCEEDED
    assert calls == Counter({"two": 2, "one": 1, "three": 1})
    assert len(interrupted_dispatches) == 2
    assert interrupted_dispatches[0] == interrupted_dispatches[1]
    final_state = resumed_ledger.reconstruct()
    assert final_state.status == "completed"
    final_events = resumed_ledger.load_events()
    assert next(event for event in final_events if event.event_id == completed_event.event_id).sha256 == completed_event.sha256
    ...
    for source, content in completed_bindings.items():
        assert (root / source).read_bytes() == content
    checkpoint_after = resumed_ledger.load_checkpoint()
