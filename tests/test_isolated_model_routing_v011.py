...
def _git(root: Path, *args: str) -> str:
    completed = subprocess.run(["git", *args], cwd=root, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8", check=True, shell=False)
...
"""
    ...
    context = IsolatedContextSlice(source=f"specs/changes/{FEATURE}/requirements.md", line_start=1, line_end=3, source_sha256="sha256:" + sha256(raw).hexdigest(), text="# Requirements\n\n- FR-001: Preserve café behavior.")
    contract = IsolatedTaskContract(feature_id=FEATURE, task_id="REMEDIATE-ROUTING-123", remediation_task_sha256="sha256:" + "1" * 64, round_id="ROUND-ROUTING-123", attempt=1, stage=IsolatedStage.IMPLEMENT, git_commit=head, dispatch_id="dispatch-routing-123", semantic_agent="developer", capability=Capability.CODING, mode=ExecutionMode.WORKSPACE_WRITE, summary="Implement FR-001.", allowed_roots=("src", "tests"), forbidden_roots=(f"specs/changes/{FEATURE}/requirements.md", "specs/current"), context=(context,))
    request = routing_request_for_isolated_contract(contract, risk="standard", complexity="medium", affected_technologies=("python",), provider_availability={"provider-a": True, "provider-b": True})
    routed = build_routed_isolated_invocation(AgentRuntime(root), contract, request)
