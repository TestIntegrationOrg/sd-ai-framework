...
def _approve(root: Path) -> None:
    ...
    record = TraceEvidence(
        ...
        bindings=(
            EvidenceBinding(EvidenceBindingKind.ARTIFACT, topology.source, topology.file_sha256),
        ),
...
""",
    ...
    _write(
        root / "src" / "api" / "check.ps1",
        "Invoke-RestMethod -Uri 'https://data.internal/health' -Method Get\n",
    )
    ...
    assert len(outgoing.provenance) == 2
...
def test_contract_symbol_change_is_reported_as_required_missing_and_unexpected_current_binding(tmp_path: Path) -> None:
    root = _project(tmp_path)
    approved, first = _observe(root)
    assert any(item.kind is ArchitectureFactKind.CONTRACT for item in first.facts)
...
""",
    ...
    current = ServiceCommunicationObserver().observe(root, approved)
    report = compare_architecture(approved, (current,))
    ...
    unexpected = next(item for item in contract_findings if item.code == "ARCH-DRIFT-UNEXPECTED-PRESENT")
    assert unexpected.observed_provenance[0].source == "contracts/public-api.yaml"


def test_removed_contract_symbol_leaves_required_contract_binding_missing(tmp_path: Path) -> None:
    root = _project(tmp_path)
    approved = load_approved_architecture(root, FEATURE)
    _write(
        root / "contracts" / "public-api.yaml",
...
""",
    ...
    observation = ServiceCommunicationObserver().observe(root, approved)
    report = compare_architecture(approved, (observation,))
    contract_findings = [item for item in report.findings if item.kind is ArchitectureFactKind.CONTRACT]
...
def test_dynamic_http_targets_fail_closed_for_python_and_javascript(tmp_path: Path) -> None:
...
def test_unapproved_external_http_target_is_explicit_stable_unexpected_fact(tmp_path: Path) -> None:
    root = _project(tmp_path, include_contract=False)
    ...
    external = [item for item in first.facts if item.target.startswith("external:http:")]

    assert len(external) == 1
    assert external[0].target == [item for item in second.facts if item.target.startswith("external:http:")][0].target
    report = compare_architecture(approved, (first,))
    unexpected = [item for item in report.findings if item.code == "ARCH-DRIFT-UNEXPECTED-PRESENT"]
    assert any(item.target == external[0].target for item in unexpected)
...
def test_shared_repository_index_uses_longest_component_root_for_communication_sources(tmp_path: Path) -> None:
    root = tmp_path / "ownership"
    root.mkdir()
    from sdai.architecture_drift import ArchitectureComponent
