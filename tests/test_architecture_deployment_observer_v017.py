...
def _sources(root: Path, *entries: tuple[str, str, str, str]) -> None:
    body = "".join(
        f"""  - id: {source_id}
    kind: {kind}
    path: {path}
...
"""
        for source_id, kind, path, environment in entries
    )
...
def test_kubernetes_workload_public_service_ports_and_secret_redaction(tmp_path: Path) -> None:
    ...
    workload_attrs = {
        ...
        "name": "api", "direction": "placement", "ports": ["TCP:8080"],
    }
    exposure_attrs = {
        ...
        "ports": ["TCP:443->8080"], "resource": service,
    }
    facts = _fact("API-W", "deployment", "required", "api", workload, workload_attrs)
    facts += _fact("API-X", "deployment", "required", "external:public", "api", exposure_attrs)
    root = _project(tmp_path, facts)
    _sources(root, ("prod-k8s", "kubernetes", "deploy/prod.yaml", "prod"))
...
apiVersion: v1
kind: Service
metadata: {name: api-public, namespace: apps, labels: {sdai.io/component: api}}
...
def test_kubernetes_exposure_change_and_ingress_resolution(tmp_path: Path) -> None:
    service = deployment_subject_id("prod-k8s", "kubernetes", "apps", "Service", "api")
    internal_attrs = {
        "role": "exposure", "platform": "kubernetes", "sourceId": "prod-k8s",
        "environment": "prod", "namespace": "apps", "resourceKind": "Service",
        "name": "api", "exposure": "internal", "direction": "inbound",
        "ports": ["TCP:80"], "resource": service,
    }
    root = _project(tmp_path / "exposure", _fact("API-SVC", "deployment", "required", "api", "api", internal_attrs))
    _sources(root, ("prod-k8s", "kubernetes", "deploy/service.yaml", "prod"))
...
""")
    ...
    assert {item.code for item in report.findings if item.kind is ArchitectureFactKind.DEPLOYMENT} == {
        "ARCH-DRIFT-REQUIRED-MISSING", "ARCH-DRIFT-UNEXPECTED-PRESENT"
    }

    ingress_root = _project(tmp_path / "ingress")
    _sources(ingress_root, ("k8s", "kubernetes", "deploy/ingress.yaml", "prod"))
    _write(ingress_root / "deploy" / "ingress.yaml", """apiVersion: networking.k8s.io/v1
kind: Ingress
...
spec: {ports: [{port: 80}]}
""")
    _, ingress_observation = _observe(ingress_root)
    ingress = next(item for item in ingress_observation.facts if dict(item.attributes).get("resourceKind") == "Ingress")
...
def test_ingress_with_any_unresolved_backend_fails_closed(tmp_path: Path) -> None:
    root = _project(tmp_path)
    _sources(root, ("k8s", "kubernetes", "deploy/ingress.yaml", "prod"))
...
def test_compose_dependency_public_port_and_secret_redaction(tmp_path: Path) -> None:
    ns = "compose:local"
    ...
    api_w = {"role": "workload", "platform": "compose", "sourceId": "local", "environment": "dev", "namespace": ns, "workloadKind": "service", "name": "api", "direction": "placement", "ports": ["TCP:8080@8080"]}
    ...
    exposure = {"role": "exposure", "platform": "compose", "sourceId": "local", "environment": "dev", "namespace": ns, "resourceKind": "service", "name": "api", "exposure": "public", "direction": "inbound", "ports": ["TCP:8080@8080"], "resource": api_target}
    ...
    facts += _fact("API-D", "deployment", "required", "api", "data", dependency)
    root = _project(tmp_path, facts)
    _sources(root, ("local", "compose", "compose.yaml", "dev"))
...
def test_terraform_metadata_dependency_comments_and_secret_safety(tmp_path: Path) -> None:
    ...
    api_w = {"role": "workload", "platform": "terraform", "sourceId": "infra", "environment": "prod", "namespace": "apps", "workloadKind": "aws_ecs_service", "name": "api", "direction": "placement", "ports": ["HTTPS:443"]}
    ...
    exposure = {"role": "exposure", "platform": "terraform", "sourceId": "infra", "environment": "prod", "namespace": "apps", "resourceKind": "aws_ecs_service", "name": "api", "exposure": "public", "direction": "inbound", "ports": ["HTTPS:443"], "resource": api_target}
    ...
    facts = _fact("TF-A", "deployment", "required", "api", api_target, api_w)
    facts += _fact("TF-D", "deployment", "required", "data", data_target, data_w)
    ...
    root = _project(tmp_path, facts)
    _sources(root, ("infra", "terraform", "infra/main.tf", "prod"))
...
""")
    ...
    assert "TERRAFORMSECRET" not in payload
    assert "NEVEREMIT" not in payload
    assert '"https://example.invalid/a#b//c"' in _strip_hcl_comments('value = "https://example.invalid/a#b//c" # comment\n')


def test_colocation_isolation_path_safety_and_component_ambiguity(tmp_path: Path) -> None:
    ns = "compose:local"
    api_target = deployment_subject_id("local", "compose", ns, "service", "api")
    data_target = deployment_subject_id("local", "compose", ns, "service", "data")
    ...
    root = _project(tmp_path / "constraints", facts)
    _sources(root, ("local", "compose", "compose.yaml", "dev"))
    ...
    for path in ("../outside.yaml", "C:/deploy.yaml"):
        _sources(unsafe, ("bad", "kubernetes", path, "prod"))
        with pytest.raises(ArchitectureDriftError, match="SDAI-ARCH-DEPLOY-002"):
            load_deployment_sources(unsafe)

    ambiguous = _project(tmp_path / "ambiguous")
    _sources(ambiguous, ("api-k8s", "kubernetes", "src/api/deploy.yaml", "prod"))
    _write(ambiguous / "src" / "api" / "deploy.yaml", """apiVersion: apps/v1
kind: Deployment
...
def test_deployment_placement_and_public_exposure_feed_security_engine(tmp_path: Path) -> None:
    workload = deployment_subject_id("prod-k8s", "kubernetes", "prod-ns", "Deployment", "api")
    service = deployment_subject_id("prod-k8s", "kubernetes", "prod-ns", "Service", "api-public")
    workload_attrs = {"role": "workload", "platform": "kubernetes", "sourceId": "prod-k8s", "environment": "prod", "namespace": "prod-ns", "workloadKind": "Deployment", "name": "api", "direction": "placement", "ports": []}
    exposure_attrs = {"role": "exposure", "platform": "kubernetes", "sourceId": "prod-k8s", "environment": "prod", "namespace": "prod-ns", "resourceKind": "Service", "name": "api-public", "exposure": "public", "direction": "inbound", "ports": ["TCP:443"], "resource": service}
    facts = _fact("API-W", "deployment", "allowed", "api", workload, workload_attrs)
    facts += _fact("API-X", "deployment", "allowed", "external:public", "api", exposure_attrs)
    facts += _fact("ZONE-API", "trust-boundary", "allowed", "api", "zone:internal", {"role": "zone-membership"})
    facts += _fact("ZONE-W", "trust-boundary", "allowed", workload, "zone:privileged", {"role": "zone-membership"})
    facts += _fact("PLACEMENT", "trust-boundary", "allowed", "zone:internal", "zone:privileged", {"role": "boundary-rule", "evidenceKind": "deployment", "direction": "placement"})
    facts += _fact("EXPOSURE", "trust-boundary", "allowed", "zone:external", "zone:internal", {"role": "boundary-rule", "evidenceKind": "deployment", "direction": "inbound"})
    root = _project(tmp_path, facts)
    _sources(root, ("prod-k8s", "kubernetes", "deploy/api.yaml", "prod"))
...
""")
    ...
    report = evaluate_trust_boundary_security(approved, (deployment,))
    assert [item for item in report.findings if item.kind is ArchitectureFactKind.TRUST_BOUNDARY] == []
