...
def _add_common(parser: argparse.ArgumentParser, *, version: bool = False, registry: bool = False) -> None:
    if version:
...
def add_integration_parser(commands: argparse._SubParsersAction) -> None:
    ...
    install = actions.add_parser("install", help="Materialize an Integration without silently changing versions")
    ...
    status = actions.add_parser("status", help="Compare installed state with the desired resolved Integration")
    status.add_argument("integration_id")
    _add_common(status, version=True, registry=True)

    repair = actions.add_parser("repair", help="Repair the installed exact Integration version")
    repair.add_argument("integration_id")
    _add_common(repair, version=True, registry=True)

    upgrade = actions.add_parser("upgrade", help="Upgrade an installed Integration to latest or an exact version")
    upgrade.add_argument("integration_id")
    _add_common(upgrade, version=True, registry=True)

    remove = actions.add_parser("remove", help="Remove managed files while preserving user-modified content")
...
def _registry_sources(root: Path, args: argparse.Namespace) -> tuple[IntegrationSource, ...]:
    ...
    user_default = _resolve_path(root, user_env) if user_env else Path.home() / ".sdai" / "integrations"
...
def build_cli_integration_registry(root: Path, args: argparse.Namespace) -> IntegrationRegistry:
...
def _selection_path(root: Path) -> Path:
    ...
    try:
        ...
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection escapes the project root") from exc
    current = root
    for part in Path(_SELECTION_RELATIVE).parts:
        current = current / part
        if current.is_symlink():
            raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection path must not contain symlinks")
    return candidate
...
class IntegrationSelection:
    ...
    @classmethod
    def from_installed(cls, record: InstalledIntegration) -> "IntegrationSelection":
        return cls(
            id=record.id,
    ...
    def from_dict(cls, value: object) -> "IntegrationSelection":
        ...
        if not isinstance(value, Mapping) or set(value) != expected:
            raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection record is invalid")
        provenance = value["provenance"]
        if not isinstance(provenance, Mapping) or set(provenance) != {"layer", "path", "source"}:
            raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection provenance is invalid")
        scalar = (value["id"], value["identity"], value["manifestSha256"], value["version"])
        provenance_scalar = (provenance["layer"], provenance["path"], provenance["source"])
        if not all(isinstance(item, str) and item for item in (*scalar, *provenance_scalar)):
            raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection fields must be non-empty strings")
        if value["identity"] != f"{value['id']}@{value['version']}":
            raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection identity/version mismatch")
        return cls(
            id=value["id"],
...
def _load_selection(root: Path) -> IntegrationSelection | None:
    ...
    if path.is_symlink() or not path.is_file():
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection must be a regular file")
    try:
        value = json.loads(path.read_text(encoding="utf-8", errors="strict"))
    except (OSError, UnicodeError, json.JSONDecodeError) as exc:
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection must be valid UTF-8 JSON") from exc
    if not isinstance(value, Mapping) or set(value) != {"apiVersion", "selection"}:
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection contract is invalid")
    if value["apiVersion"] != INTEGRATION_SELECTION_API_VERSION:
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection apiVersion is unsupported")
...
def _write_selection(root: Path, selection: IntegrationSelection) -> None:
    ...
    if path.parent.is_symlink() or path.is_symlink():
        raise _fail("SDAI-INTEGRATION-CLI-003", "Integration selection path must not be a symlink")
    data = (_canonical_json({"apiVersion": INTEGRATION_SELECTION_API_VERSION, "selection": selection.as_dict()}) + "\n").encode("utf-8")
    ...
    try:
        ...
        raise _fail("SDAI-INTEGRATION-CLI-003", "unable to atomically persist Integration selection") from exc
    finally:
...
def _clear_selection_if(root: Path, integration_id: str) -> bool:
    selection = _load_selection(root)
    if selection is None or selection.id != integration_id:
        return False
    try:
        _selection_path(root).unlink(missing_ok=True)
    except OSError as exc:
        raise _fail("SDAI-INTEGRATION-CLI-003", "unable to clear Integration selection") from exc
    return True


def _selected(root: Path, integration_id: str, identity: str | None = None) -> bool:
    selection = _load_selection(root)
    return bool(
...
def _emit_result(args: argparse.Namespace, payload: dict[str, object], *, human: str) -> None:
    if args.json:
        _emit_json(payload)
    else:
        print(human)
...
def _not_found(args: argparse.Namespace, action: str, integration_id: str, *, detail: str) -> int:
...
def _run_info(root: Path, args: argparse.Namespace) -> int:
    registry = build_cli_integration_registry(root, args)
    resolved = _find_resolution(registry, args.integration_id, args.version)
    ...
    payload = {
        ...
        "selected": _selected(root, args.integration_id, None if resolved is None else resolved.identity),
    }
    if args.json:
        _emit_json(payload)
    elif resolved is not None:
        print(
            f"{resolved.identity} layer={resolved.selected_provenance.layer.value} "
            f"source={resolved.selected_provenance.source}/{resolved.selected_provenance.path}"
        )
    else:
...
def _run_install(root: Path, args: argparse.Namespace) -> int:
    registry = build_cli_integration_registry(root, args)
    desired = _find_resolution(registry, args.integration_id, args.version)
    if desired is None:
        return _not_found(args, "install", args.integration_id, detail="requested version is not available")
    ...
    if existing is not None and existing.identity != desired.identity:
        ...
        _emit_result(
            args,
            payload,
            human=f"Integration {existing.identity} is already installed; use `sdai integration upgrade` for {desired.identity}",
        )
        return EXIT_ACTION_REQUIRED
    record = materialize_integration(root, desired)
    payload = _result_payload("install", "ok", args.integration_id, record=record, desired=desired)
...
def _run_use(root: Path, args: argparse.Namespace) -> int:
    ...
    if record is None:
        return _not_found(args, "use", args.integration_id, detail="it is not installed")
    if args.version is not None and record.version != args.version:
        payload = _result_payload("use", "different-version-installed", args.integration_id, record=record)
        _emit_result(
            args,
            payload,
            human=f"Installed Integration is {record.identity}, not {args.integration_id}@{args.version}",
        )
        return EXIT_ACTION_REQUIRED
...
def _run_status(root: Path, args: argparse.Namespace) -> int:
    ...
    if desired is None:
        if installed is None:
            ...
            if args.json:
                ...
                print(f"Integration {args.integration_id} is neither installed nor available")
            return EXIT_NOT_FOUND
        payload = {
            "apiVersion": INTEGRATION_STATUS_COMMAND_API_VERSION,
        ...
        if args.json:
            ...
            print(f"Integration {installed.identity} is installed but no matching registry target is available")
        return EXIT_ACTION_REQUIRED
    ...
    payload = {
        ...
        "selected": _selected(root, args.integration_id, None if installed is None else installed.identity),
        "status": report.status.value,
    }
    if args.json:
        _emit_json(payload)
    else:
        print(f"Integration {args.integration_id} status={report.status.value} desired={desired.identity}")
        for finding in report.findings:
            path = finding.path or "<integration>"
            print(f"  {finding.status.value:18} {path} - {finding.detail}")
    return EXIT_OK if report.status == IntegrationFileStatus.EXACT else EXIT_ACTION_REQUIRED


def _run_repair(root: Path, args: argparse.Namespace) -> int:
    record = _installed(root, args.integration_id)
    if record is None:
        return _not_found(args, "repair", args.integration_id, detail="it is not installed")
    if args.version is not None and record.version != args.version:
        payload = _result_payload("repair", "different-version-installed", args.integration_id, record=record)
        _emit_result(args, payload, human=f"Repair targets installed {record.identity}; use upgrade to change versions")
        return EXIT_ACTION_REQUIRED
    registry = build_cli_integration_registry(root, args)
    desired = registry.resolve(args.integration_id, record.version)
    if desired is None:
        return _not_found(args, "repair", args.integration_id, detail=f"installed exact version {record.version} is unavailable")
    repaired = repair_integration(root, desired)
    payload = _result_payload("repair", "ok", args.integration_id, record=repaired, desired=desired)
...
def _run_upgrade(root: Path, args: argparse.Namespace) -> int:
    existing = _installed(root, args.integration_id)
    if existing is None:
        return _not_found(args, "upgrade", args.integration_id, detail="it is not installed; use install first")
    registry = build_cli_integration_registry(root, args)
    desired = _find_resolution(registry, args.integration_id, args.version)
    if desired is None:
        return _not_found(args, "upgrade", args.integration_id, detail="requested target version is unavailable")
    upgraded = materialize_integration(root, desired)
    payload = _result_payload("upgrade", "ok", args.integration_id, record=upgraded, desired=desired)
    _emit_result(args, payload, human=f"Upgraded Integration {existing.identity} -> {upgraded.identity}")
    return EXIT_OK
...
def _run_remove(root: Path, args: argparse.Namespace) -> int:
    ...
    selection_cleared = _clear_selection_if(root, args.integration_id)
    payload = _result_payload(
        "remove",
        "ok",
        args.integration_id,
...
def _error_code(exc: BaseException) -> str:
    ...
    return prefix if prefix.startswith("SDAI-") else "SDAI-INTEGRATION-CLI-999"
...
def run_integration_command(root: Path, args: argparse.Namespace) -> int:
    try:
        ...
        raise _fail("SDAI-INTEGRATION-CLI-001", f"unknown Integration action '{action}'")
    except (
        IntegrationCliError,
