...
_LOCK_ANCHOR = b"SDAI-LEDGER-LOCK-v1\n"
...
def _write_all(fd: int, content: bytes) -> None:
    ...
    while view:
        ...
        if written <= 0:
            raise _fail("SDAI-LEDGER-007", "operating system returned a short/zero write")
        view = view[written:]
...
def _require_string(value: object, *, label: str, code: str = "SDAI-LEDGER-004") -> str:
    if not isinstance(value, str):
        raise _fail(code, f"{label} must be a string")
    return value
...
def _validate_portable_source(source: str) -> str:
    ...
    if "\\" in source or source.startswith("/") or re.match(r"^[A-Za-z]:", source):
        raise _fail(
            "SDAI-LEDGER-002",
            f"binding source must be repository-relative POSIX path: {source!r}",
        )
    path = PurePosixPath(source)
    if any(part in {"", ".", ".."} for part in path.parts):
        raise _fail(
            "SDAI-LEDGER-002",
            f"binding source is not a safe repository-relative path: {source!r}",
        )
...
class HashBinding:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.source, str):
            raise _fail("SDAI-LEDGER-002", "binding source must be a string")
    ...
    def from_mapping(cls, raw: object) -> "HashBinding":
        if not isinstance(raw, Mapping) or set(raw) != {"kind", "source", "sha256"}:
            raise _fail(
                "SDAI-LEDGER-004",
                "event binding must contain exactly kind/source/sha256",
            )
        kind = _require_string(raw["kind"], label="binding kind")
        source = _require_string(raw["source"], label="binding source")
        digest = _require_string(raw["sha256"], label="binding sha256")
...
class LedgerEvent:
    ...
    def from_mapping(cls, raw: object) -> "LedgerEvent":
        if not isinstance(raw, Mapping) or set(raw) != _EVENT_KEYS:
            raise _fail(
                "SDAI-LEDGER-004",
                "event record fields do not match sdai.execution-event/v1",
            )
        ...
        _validate_json_value(payload, label="event payload")
        ...
        binding_keys = [(item.kind, item.source) for item in bindings]
        if len(binding_keys) != len(set(binding_keys)):
            raise _fail(
                "SDAI-LEDGER-004",
                "event contains duplicate binding kind/source entries",
            )
        git_raw = raw.get("git_commit")
        if git_raw is not None and not isinstance(git_raw, str):
            raise _fail("SDAI-LEDGER-004", "event git_commit must be a string or null")
        git_commit = _validate_commit(git_raw)
        previous = _require_string(raw.get("previous_sha256"), label="event previous_sha256")
        digest = _require_string(raw.get("sha256"), label="event sha256")
        if not _SHA256.fullmatch(previous):
            raise _fail("SDAI-LEDGER-004", "event previous_sha256 is invalid")
        if not _SHA256.fullmatch(digest):
            raise _fail("SDAI-LEDGER-004", "event sha256 is invalid")
        run_id = _require_string(raw.get("run_id"), label="event run_id")
        feature_raw = _require_string(raw.get("feature_id"), label="event feature_id")
        event_id = _require_string(raw.get("event_id"), label="event event_id")
        recorded_at = _require_string(raw.get("recorded_at"), label="event recorded_at")
        if not _RUN_ID.fullmatch(run_id):
            raise _fail("SDAI-LEDGER-004", "event run_id is invalid")
        feature_id = validate_feature_id(feature_raw)
...
class _LedgerLock:
    """Cross-platform advisory lock whose ownership dies with the process.

    The lock file is a persistent anchor. OS lock ownership, not file existence,
    controls exclusivity, so a hard process exit cannot leave the run bricked.
    """
    ...
    def __enter__(self) -> "_LedgerLock":
        if self.path.exists() and self.path.is_symlink():
            raise _fail("SDAI-LEDGER-003", "ledger.lock must not be a symlink")
        ...
        try:
            ...
            raise _fail("SDAI-LEDGER-006", f"unable to open ledger lock anchor: {exc}") from exc
        try:
            ...
            os.lseek(fd, 0, os.SEEK_SET)
            prefix = os.read(fd, 32)
            if prefix.startswith(b"pid="):
                self._release_os_lock(fd)
                raise _fail(
                    "SDAI-LEDGER-006",
                    "execution ledger is locked by another process (legacy/manual lock marker)",
                )
            self.fd = fd
            return self
        except Exception:
    ...
    def _acquire_os_lock(self, fd: int) -> None:
        try:
            if self._windows:
                import msvcrt
...
class ExecutionLedger:
    ...
    def binding_for_file(self, path: Path, *, kind: str = "artifact") -> HashBinding:
        candidate = _safe_existing_path(
            self.project_root,
            path,
            label="execution evidence file",
        )
        if candidate.is_symlink() or not candidate.is_file():
            raise _fail(
                "SDAI-LEDGER-003",
                f"binding source must be a regular non-symlink file: {candidate}",
            )
    ...
    def write_task_brief(self, task_id: str, content: str) -> HashBinding:
        ...
        with self._lock():
            ...
            current = state.task_map().get(task)
            if current is None:
    ...
    def write_task_record(
        ...
        if record_type not in {"implementation", "review", "evidence"}:
            raise _fail(
                "SDAI-LEDGER-002",
                f"unsupported task record type: {record_type!r}",
            )
    ...
    def append_event(
        ...
        *,
        ...
        if len(keys) != len(set(keys)):
            raise _fail(
                "SDAI-LEDGER-002",
                "event bindings contain duplicate kind/source entries",
            )
        with self._lock():
    ...
    def _append_line_unlocked(self, event: LedgerEvent) -> None:
        ...
        try:
            ...
            raise _fail("SDAI-LEDGER-007", f"unable to open append ledger: {exc}") from exc
        try:
            _write_all(fd, line)
            os.fsync(fd)
        finally:
    ...
    def _load_events_unlocked(self) -> tuple[LedgerEvent, ...]:
        ...
        if self.events_path.is_symlink() or not self.events_path.is_file():
            raise _fail(
                "SDAI-LEDGER-003",
                "events.jsonl must be a regular non-symlink file",
            )
        try:
            raw = self.events_path.read_bytes()
        except OSError as exc:
            raise _fail("SDAI-LEDGER-004", f"unable to read events.jsonl: {exc}") from exc
        if raw and not raw.endswith(b"\n"):
            raise _fail(
                "SDAI-LEDGER-004",
                "events.jsonl ends with a truncated/incomplete record",
            )
        ...
        for line_number, line in enumerate(raw.splitlines(), start=1):
            if not line.strip():
                raise _fail(
                    "SDAI-LEDGER-004",
                    f"events.jsonl contains blank record at line {line_number}",
                )
            try:
            ...
            if event.sequence != expected_sequence:
                raise _fail(
                    "SDAI-LEDGER-004",
                    f"event sequence gap/duplicate at line {line_number}: "
                    f"expected {expected_sequence}, got {event.sequence}",
                )
            if (
                event.run_id != self.manifest.run_id
                or event.feature_id != self.manifest.feature_id
            ):
                raise _fail(
                    "SDAI-LEDGER-004",
                    f"event identity mismatch at line {line_number}",
                )
            if event.previous_sha256 != previous:
                raise _fail(
                    "SDAI-LEDGER-004",
                    f"event hash chain mismatch at line {line_number}",
                )
            expected_hash = _sha256_bytes(_canonical_bytes(event.body_dict()))
            if event.sha256 != expected_hash:
                raise _fail(
                    "SDAI-LEDGER-004",
                    f"event content hash mismatch at line {line_number}",
                )
            events.append(event)
            previous = event.sha256
        result = tuple(events)
        self._reconstruct_events(result)
        return result
    ...
    def _validate_transition(
        ...
        if state.status in _RUN_TERMINAL:
            raise _fail(
                "SDAI-LEDGER-005",
                f"run is already terminal ({state.status})",
            )
        ...
        if kind == "run.created":
            if state.last_sequence != 0:
                raise _fail(
                    "SDAI-LEDGER-005",
                    "run.created may appear only as the first event",
                )
            return
        if state.last_sequence == 0:
            raise _fail("SDAI-LEDGER-005", "run.created must be the first event")
        if state.status == "paused" and kind not in {
            "run.resumed",
            "run.failed",
            "run.cancelled",
        }:
            raise _fail(
                "SDAI-LEDGER-005",
                "paused run must be resumed before additional task/run activity",
            )
        if kind == "run.paused":
            if state.status != "active":
                raise _fail(
                    "SDAI-LEDGER-005",
                    f"run cannot pause from status {state.status}",
                )
            return
        if kind == "run.resumed":
            if state.status != "paused":
                raise _fail(
                    "SDAI-LEDGER-005",
                    f"run cannot resume from status {state.status}",
                )
            return
        ...
        if kind.startswith("task."):
            ...
            if current.status != "started":
                raise _fail(
                    "SDAI-LEDGER-005",
                    f"event '{kind}' requires task '{task_id}' to be started",
                )
            if kind == "task.completed":
                ...
                if not bindings:
                    raise _fail(
                        "SDAI-LEDGER-005",
                        f"task '{task_id}' completion requires at least one "
                        "artifact/evidence hash binding",
                    )
            return
        if kind == "run.completed":
            ...
            if incomplete:
                raise _fail(
                    "SDAI-LEDGER-005",
                    "run cannot complete while tasks are incomplete: "
                    + ", ".join(incomplete),
                )
    ...
    def _reconstruct_events(
        ...
        for index, event in enumerate(events):
            ...
            if event.previous_sha256 != previous:
                raise _fail(
                    "SDAI-LEDGER-004",
                    "event hash chain is not contiguous",
                )
    ...
    def _reconstruct_unlocked(self) -> ExecutionState:
        return self._reconstruct_events(self._load_events_unlocked())
    ...
    def load_checkpoint(self) -> dict[str, object] | None:
        with self._lock():
            ...
            if self.checkpoint_path.is_symlink() or not self.checkpoint_path.is_file():
                raise _fail(
                    "SDAI-LEDGER-003",
                    "checkpoint.json must be a regular non-symlink file",
                )
            try:
                payload = json.loads(
            ...
            if not isinstance(payload, Mapping) or set(payload) != _CHECKPOINT_KEYS:
                raise _fail(
                    "SDAI-LEDGER-004",
                    "invalid execution checkpoint fields",
                )
            if payload.get("apiVersion") != "sdai.execution-checkpoint/v1":
                raise _fail(
                    "SDAI-LEDGER-004",
                    "invalid execution checkpoint apiVersion",
                )
            ...
            state = self._reconstruct_unlocked()
            if (
                payload.get("run_id") != self.manifest.run_id
                or payload.get("feature_id") != self.manifest.feature_id
            ):
                raise _fail(
                    "SDAI-LEDGER-004",
                    "checkpoint run/feature identity mismatch",
                )
            if (
                payload.get("last_sequence") != state.last_sequence
                or payload.get("last_sha256") != state.last_sha256
            ):
                raise _fail(
                    "SDAI-LEDGER-004",
                    "checkpoint is stale relative to the current event ledger",
                )
            if payload.get("state") != state.as_dict():
                raise _fail(
                    "SDAI-LEDGER-004",
                    "checkpoint state does not match reconstructed ledger state",
                )
            if not isinstance(payload.get("extra"), Mapping):
                raise _fail(
                    "SDAI-LEDGER-004",
                    "checkpoint extra must be a mapping",
                )
            return dict(payload)
...
def _manifest_from_mapping(raw: object) -> RunManifest:
    ...
    if set(raw) != expected or raw.get("apiVersion") != "sdai.execution-run/v1":
        raise _fail(
            "SDAI-LEDGER-004",
            "run.json fields do not match sdai.execution-run/v1",
        )
    run_id = _require_string(raw.get("run_id"), label="run.json run_id")
    feature_raw = _require_string(
        raw.get("feature_id"),
        label="run.json feature_id",
    )
    workflow = _require_string(raw.get("workflow"), label="run.json workflow")
    baseline_raw = _require_string(
        raw.get("baseline_commit"),
        label="run.json baseline_commit",
    )
    created_raw = _require_string(
        raw.get("created_at"),
        label="run.json created_at",
    )
    if not _RUN_ID.fullmatch(run_id):
    ...
    feature = validate_feature_id(feature_raw)
    baseline = _validate_commit(baseline_raw, required=True)
    assert baseline is not None
    created_at = _validate_timestamp(created_raw)
...
def create_execution_run(
    ...
    *,
    ...
    if not feature_dir.is_dir() or feature_dir.is_symlink():
        raise _fail(
            "SDAI-LEDGER-001",
            "feature directory must already exist before creating a run: "
            f"specs/{feature}",
        )
    created = _now()
    generated = run_id or (
        f"run-{created.strftime('%Y%m%dT%H%M%S%fZ')}-{uuid4().hex[:12]}"
    )
    ...
    try:
        ...
        raise _fail(
            "SDAI-LEDGER-001",
            f"execution run already exists: {generated}",
        ) from exc
...
def load_execution_run(
    ...
    if manifest_path.is_symlink() or not manifest_path.is_file():
        raise _fail(
            "SDAI-LEDGER-001",
            f"execution run manifest does not exist: {run_id}",
        )
    try:
    ...
    if manifest.feature_id != feature or manifest.run_id != run_id:
        raise _fail(
            "SDAI-LEDGER-004",
            "run.json path identity does not match manifest identity",
        )
    ledger = ExecutionLedger(root, manifest)
    ...
    if not events or events[0].kind != "run.created":
        raise _fail(
            "SDAI-LEDGER-004",
            "execution run is missing its run.created ledger event",
        )
    created = events[0]
    current_manifest_binding = ledger.binding_for_file(
        manifest_path,
        kind="evidence",
    )
    if current_manifest_binding not in created.bindings:
        raise _fail(
            "SDAI-LEDGER-004",
            "run.json byte identity does not match run.created evidence binding",
        )
    if created.git_commit != manifest.baseline_commit:
        raise _fail(
            "SDAI-LEDGER-004",
            "run.created Git baseline does not match run.json",
        )
    if created.payload.get("workflow") != manifest.workflow:
        raise _fail(
            "SDAI-LEDGER-004",
            "run.created workflow does not match run.json",
        )
    if created.payload.get("manifest_sha256") != manifest.sha256:
        raise _fail(
            "SDAI-LEDGER-004",
            "run.created manifest hash does not match run.json",
        )
    return ledger
