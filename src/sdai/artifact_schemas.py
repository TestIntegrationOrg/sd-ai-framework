...
def _validate_path(value: object, *, label: str) -> str:
    if not isinstance(value, str) or not value.strip():
        raise _fail("SDAI-SCHEMA-002", f"{label} must be a non-empty POSIX repository-relative path")
    text = value.strip()
    if "\\" in text or re.match(r"^[A-Za-z]:", text) or text.startswith("/"):
        raise _fail("SDAI-SCHEMA-002", f"{label} must be a POSIX repository-relative path")
    path = PurePosixPath(text)
    if any(part in {"", ".", ".."} for part in path.parts):
        raise _fail("SDAI-SCHEMA-002", f"{label} must not contain dot/parent segments")
    ...
    if "{" in scrubbed or "}" in scrubbed:
        raise _fail("SDAI-SCHEMA-002", f"{label} contains malformed placeholder syntax")
    return text


def _manifest_from_file(path: Path, *, source: str) -> object:
    try:
        raw = yaml.safe_load(read_utf8_text(path)) or {}
    except (OSError, TextEncodingError, yaml.YAMLError) as exc:
        raise _fail("SDAI-SCHEMA-001", f"unable to read artifact schema {source}: {exc}") from exc
    if not isinstance(raw, Mapping):
        raise _fail("SDAI-SCHEMA-001", f"artifact schema {source} must be a YAML mapping")
    try:
        manifest = parse_extension_manifest(raw, source=source)
    except ExtensionManifestError as exc:
        raise _fail("SDAI-SCHEMA-001", f"invalid artifact schema {source}: {exc}") from exc
...
def _parse_manifest_overlays(
    ...
    *,
    ...
    if not isinstance(raw_artifacts, list):
        raise _fail("SDAI-SCHEMA-001", f"artifact schema {source} spec.artifacts must be a list")
    ...
    for index, raw in enumerate(raw_artifacts, start=1):
        ...
        if not isinstance(artifact_id, str) or not _ARTIFACT_ID.fullmatch(artifact_id) or ".." in artifact_id:
            raise _fail(
                "SDAI-SCHEMA-001",
        ...
        if "path" in fields:
            fields["path"] = _validate_path(fields["path"], label=f"artifact '{artifact_id}' path")
        if "type" in fields:
        ...
        if "depends_on" in fields:
            dependencies = _string_list(
                fields["depends_on"], label=f"artifact '{artifact_id}' depends_on"
            )
        ...
        if "applies_to" in fields:
            risks = _string_list(fields["applies_to"], label=f"artifact '{artifact_id}' applies_to")
...
def _paths(value: str | None, *, label: str) -> tuple[Path, ...]:
    if not value:
        return ()
    path = Path(value)
    if not path.is_absolute():
        raise _fail("SDAI-SCHEMA-008", f"{label} must be an absolute file or directory path")
    if path.is_symlink():
...
def _layer_files(
    ...
    *,
    ...
    for path in sorted(builtin.glob("*.yaml"), key=lambda item: item.name.casefold()):
        result.append((ArtifactSchemaLayer.BUILTIN, path, f"builtin:{path.name}"))

    for path in _paths(environ.get("SDAI_ORG_SCHEMA_PATH"), label="SDAI_ORG_SCHEMA_PATH"):
        result.append((ArtifactSchemaLayer.ORG, path, str(path)))

    repo_dir = ensure_within_project(root, root / ".sdai" / "schemas", label="repository schema directory")
    if repo_dir.exists():
        if repo_dir.is_symlink() or not repo_dir.is_dir():
            raise _fail("SDAI-SCHEMA-008", ".sdai/schemas must be a real directory")
        for path in sorted(
            [*repo_dir.glob("*.yaml"), *repo_dir.glob("*.yml")],
            key=lambda item: item.name.casefold(),
        ):
            if path.is_symlink():
                raise _fail("SDAI-SCHEMA-008", f"repository schema must not be a symlink: {_portable(root, path)}")
            result.append((ArtifactSchemaLayer.REPO, path, _portable(root, path)))

    for path in _paths(environ.get("SDAI_USER_SCHEMA_PATH"), label="SDAI_USER_SCHEMA_PATH"):
        result.append((ArtifactSchemaLayer.USER, path, str(path)))
    return tuple(result)
...
def _merge_overlay(
    ...
    if existing is None:
        ...
        return ArtifactDefinition(
            ...
            applies_to=tuple(fields.get("applies_to", ("standard", "critical", "regulated"))),
            source_layer=overlay.layer,
            source_schema=overlay.schema_id,
            source=overlay.source,
            history=(_contribution(overlay),),
        )
    ...
    org_contributions = [
        item for item in existing.history if item.layer is ArtifactSchemaLayer.ORG
    ]
    org_required = existing.required and bool(org_contributions)
    org_dependencies: set[str] = set()
    if org_contributions:
        # The current effective dependencies still contain every authoritative org
        # dependency because each lower-layer merge is checked below.
        org_dependencies.update(existing.depends_on)
    ...
    if org_required and not required:
        raise _fail(
            "SDAI-SCHEMA-004",
    ...
    if org_contributions and not org_dependencies.issubset(dependencies):
        missing = sorted(org_dependencies - set(dependencies))
        raise _fail(
            "SDAI-SCHEMA-004",
    ...
    return ArtifactDefinition(
        ...
        source_schema=overlay.schema_id,
        source=overlay.source,
        history=(*existing.history, _contribution(overlay)),
    )


def _topological_order(artifacts: Mapping[str, ArtifactDefinition]) -> tuple[str, ...]:
    ...
    def visit(artifact_id: str) -> None:
        ...
        if artifact_id in visiting:
            ...
            raise _fail("SDAI-SCHEMA-006", "artifact dependency cycle: " + " -> ".join(cycle))
...
def load_artifact_schema_graph(
    ...
    *,
    ...
    for layer, path, source in _layer_files(root, environ=env):
        ...
        for overlay in _parse_manifest_overlays(path, layer=layer, source=source):
