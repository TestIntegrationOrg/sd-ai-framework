...
def _special_nodes(index: FeatureArtifactIndex, root: Path) -> tuple[TraceNode, ...]:
    ...
    for item in index.files:
        ...
        for line_number, line in enumerate(text.splitlines(), start=1):
            ...
            node_type = TraceNodeType.RFC if prefix == "RFC" else TraceNodeType.COMPONENT
            nodes.append(
                TraceNode(
                    type=node_type,
                    entity_id=entity_id,
                    ...
                    metadata={},
                    provenance=_provenance(item.source, line_number, detail="explicit trace declaration"),
                )
            )
    return tuple(nodes)
...
def _repository_source_files(root: Path, feature_dir: Path) -> tuple[Path, ...]:
    ...
    for path in root.rglob("*"):
        ...
        if path.is_file() and path.suffix.casefold() in _SOURCE_SUFFIXES:
            result.append(path)
    return tuple(sorted(result, key=lambda value: value.relative_to(root).as_posix().casefold()))
...
def _cross_artifact_edges(
    ...
    for relation in index.relationships:
        ...
        if len(sources) != 1 or len(targets) != 1:
            ...
            reason = "missing" if not node_ids.get(missing.upper(), ()) else "ambiguous"
            gaps.append(
                TraceGap(
                    kind=f"{reason}-endpoint",
        ...
        edges.append(
            TraceEdge(
                ...
                source=sources[0],
                target=targets[0],
                provenance=_provenance(relation.source, relation.line, detail="explicit cross-artifact reference"),
...
def _repository_nodes_and_edges(
    ...
    for path in _repository_source_files(root, feature_dir):
        ...
        try:
            ...
            raise _fail("SDAI-TRACE-BUILD-002", f"unable to read UTF-8 source {source!r}: {exc}") from exc
        ...
        node = TraceNode(
            ...
            entity_id=entity_id,
            label=relative.name,
            metadata={"repository_file": True},
            provenance=_provenance(source, references[0][0], detail="first explicit trace reference"),
        )
        nodes.append(node)
        ...
        for line_number, referenced_id in references:
            ...
            edges.append(
                TraceEdge(
                    ...
                    source=node.node_id,
                    target=targets[0],
                    provenance=_provenance(source, line_number, detail="explicit repository source reference"),
                    metadata={},
                )
            )
...
def _evidence_candidates(feature_dir: Path) -> tuple[Path, ...]:
    ...
    return tuple(sorted(candidates, key=lambda item: item.as_posix().casefold()))
...
def _evidence_nodes_and_edges(
    ...
    for path in _evidence_candidates(feature_dir):
        ...
        evidence_node = TraceNode(
            ...
            provenance=_provenance(source, 1, detail="validated sdai.trace-evidence/v1 record"),
        )
        previous = evidence_ids.get(record.evidence_id)
        if previous is not None and previous != record.sha256:
            raise _fail(
                "SDAI-TRACE-BUILD-004",
                f"conflicting typed evidence identity {record.evidence_id!r}",
            )
        evidence_ids[record.evidence_id] = record.sha256
        ...
        edges.append(
            TraceEdge(
                ...
                provenance=_provenance(source, 1, detail="validated typed evidence subject binding"),
...
def build_feature_trace_graph(
    ...
    *,
    ...
    """Build one deterministic read-only trace graph for a feature.

    The builder performs no provider/model calls and writes no repository state.
    Relationships are created only from explicit indexed/source/evidence references.
    Unresolved relationships are returned as inspectable gaps.
    """
    ...
    known = _id_to_node_ids(initial_nodes)
    ...
    pre_evidence_nodes = tuple(nodes)
