...
_ROLE = re.compile(r"^[a-z0-9][a-z0-9._-]{0,127}$")
_MAX_TEXT = 4096
_MAX_JSON_DEPTH = 16
...
class TraceEvidenceError(RuntimeError):
    """Raised when trace evidence is malformed, ambiguous, unbound, or unsafe."""
    ...
    if isinstance(value, Mapping):
        for key, item in value.items():
            if not isinstance(key, str) or not key:
                raise _fail("SDAI-EVIDENCE-001", f"{label} keys must be non-empty strings")
...
def _canonical_bytes(value: Mapping[str, object]) -> bytes:
    _validate_json(value, label="trace evidence")
    try:
        return json.dumps(
            value,
            sort_keys=True,
...
def _json_dict(value: Mapping[str, object]) -> dict[str, object]:
    ...
    if not isinstance(raw, dict):
        raise _fail("SDAI-EVIDENCE-001", "evidence JSON mapping did not normalize to an object")
    return raw
...
def _text(value: str | None, *, label: str, required: bool = False) -> str | None:
    ...
    if len(normalized) > _MAX_TEXT or "\x00" in normalized:
        raise _fail("SDAI-EVIDENCE-001", f"{label} is invalid or too long")
    return normalized


def _evidence_id(value: str) -> str:
    if not isinstance(value, str) or not _EVIDENCE_ID.fullmatch(value) or "\\" in value:
        raise _fail("SDAI-EVIDENCE-001", f"invalid portable evidence_id: {value!r}")
    return value
...
def _command(value: Sequence[str] | None) -> tuple[str, ...]:
    if value is None:
        return ()
    ...
    result: list[str] = []
    for index, item in enumerate(value):
        if not isinstance(item, str) or not item or "\x00" in item or len(item) > _MAX_TEXT:
            raise _fail("SDAI-EVIDENCE-003", f"command[{index}] must be bounded non-empty text")
        result.append(item)
    return tuple(result)


def _provenance(values: Sequence[TraceProvenance]) -> tuple[TraceProvenance, ...]:
    items: dict[tuple[str, int], TraceProvenance] = {}
    for item in values:
        if not isinstance(item, TraceProvenance):
            raise _fail("SDAI-EVIDENCE-004", "provenance contains an invalid item")
        existing = items.get(item.location)
...
class EvidenceBinding:
    ...
    def from_mapping(cls, value: object) -> "EvidenceBinding":
        if not isinstance(value, Mapping) or set(value) != {"kind", "source", "sha256"}:
            raise _fail("SDAI-EVIDENCE-002", "binding must contain exactly kind/source/sha256")
        kind, source, digest = value["kind"], value["source"], value["sha256"]
        if not isinstance(kind, str) or not isinstance(source, str) or not isinstance(digest, str):
            raise _fail("SDAI-EVIDENCE-002", "binding fields must be strings")
...
class EvidenceProducer:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.semantic_role, str) or not _ROLE.fullmatch(self.semantic_role):
            raise _fail("SDAI-EVIDENCE-005", "producer semantic_role is invalid")
        object.__setattr__(self, "provider", _text(self.provider, label="producer provider"))
        object.__setattr__(self, "model", _text(self.model, label="producer model"))

    def as_dict(self) -> dict[str, object]:
        return {"semantic_role": self.semantic_role, "provider": self.provider, "model": self.model}
    ...
    def from_mapping(cls, value: object) -> "EvidenceProducer":
        if not isinstance(value, Mapping) or set(value) != {"semantic_role", "provider", "model"}:
            raise _fail(
                "SDAI-EVIDENCE-005",
                "producer must contain exactly semantic_role/provider/model",
            )
        role, provider, model = value["semantic_role"], value["provider"], value["model"]
        ...
        return cls(role, provider, model)
...
class TraceEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        subject = _text(self.subject, label="subject", required=True)
        ...
        canonical: dict[tuple[EvidenceBindingKind, str], EvidenceBinding] = {}
        ...
        object.__setattr__(
            ...
            tuple(
                sorted(
                    canonical.values(),
                    key=lambda item: (item.kind.value, item.source.casefold(), item.source, item.sha256),
                )
            ),
        )
        object.__setattr__(self, "provenance", _provenance(self.provenance))
        object.__setattr__(self, "result", _json_mapping(self.result, label="result"))
        object.__setattr__(self, "command", _command(self.command))
        object.__setattr__(self, "tool", _text(self.tool, label="tool"))

    def truth_dict(self) -> dict[str, object]:
        """Return the provider/model-independent evidence claim."""
        return {
            ...
            "tool": self.tool,
            "result": _json_dict(self.result),
            "provenance": [item.as_dict() for item in self.provenance],
        }

    @property
    def truth_sha256(self) -> str:
        return _sha256(_canonical_bytes(self.truth_dict()))
    ...
    @property
    def sha256(self) -> str:
        return _sha256(_canonical_bytes(self.body_dict()))
    ...
    def from_mapping(cls, value: object) -> "TraceEvidence":
        ...
        expected = {
            "apiVersion", "evidence_id", "kind", "status", "subject", "git_commit",
            "bindings", "command", "tool", "result", "provenance", "producer",
            "truth_sha256", "sha256",
        }
        ...
        scalar_names = ("evidence_id", "kind", "status", "subject", "git_commit")
        ...
        tool = value["tool"]
        if not isinstance(bindings, list) or not isinstance(command, list):
            raise _fail("SDAI-EVIDENCE-006", "bindings and command must be lists")
        if not isinstance(result, Mapping) or not isinstance(provenance, list):
            raise _fail("SDAI-EVIDENCE-006", "result must be a mapping and provenance a list")
        if tool is not None and not isinstance(tool, str):
            raise _fail("SDAI-EVIDENCE-006", "tool must be a string or null")
        try:
            parsed_provenance = tuple(TraceProvenance.from_mapping(item) for item in provenance)
        except RuntimeError as exc:
            raise _fail("SDAI-EVIDENCE-004", f"invalid evidence provenance: {exc}") from exc
        record = cls(
            ...
            bindings=tuple(EvidenceBinding.from_mapping(item) for item in bindings),
        ...
        truth = value["truth_sha256"]
        digest = value["sha256"]
        if not isinstance(truth, str) or not _SHA256.fullmatch(truth):
...
def validate_trace_evidence(value: TraceEvidence | Mapping[str, object] | str | bytes) -> TraceEvidence:
    """Validate canonical evidence for ledgers, quality gates, and integrations."""
...
def load_trace_evidence(project_root: Path, path: Path) -> TraceEvidence:
    """Load canonical UTF-8 evidence from a safe repository-relative location."""
