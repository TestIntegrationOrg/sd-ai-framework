...
_COMPONENT_REF = re.compile(
    r"^component:([a-z0-9](?:[a-z0-9._-]{0,126}[a-z0-9])?)$"
)
_EXPRESSION = re.compile(
    r"\$\{\{\s*inputs\.([a-z][a-z0-9_-]{0,63})\s*\}\}"
)
_EXACT_EXPRESSION = re.compile(
    r"^\$\{\{\s*inputs\.([a-z][a-z0-9_-]{0,63})\s*\}\}$"
)
    ...
    *,
    ...
    for raw_name in sorted(raw):
        ...
        if not isinstance(spec, Mapping):
            raise _fail(
                "SDAI-WFCOMP-002",
                f"{label} input '{name}' must be a mapping",
            )
        ...
        if not isinstance(raw_enum, list):
            raise _fail(
                "SDAI-WFCOMP-002",
                f"{label} input '{name}' enum must be a list",
            )
        ...
        for item in raw_enum:
            enum.append(
                _validate_value(
                    item,
                    preliminary,
                    label=f"{label} input '{name}' enum value",
                )
            )
        definition = TypedInputDefinition(
            name=name,
        ...
        if has_default:
            default = _validate_value(
                default,
                definition,
                label=f"{label} input '{name}' default",
            )
            definition = TypedInputDefinition(
                name=name,
                type=input_type,
...
def _resolve_inputs(
    ...
    *,
    ...
    for name, definition in definitions_by_name.items():
        if name in values:
            resolved[name] = _validate_value(
                values[name],
                definition,
                label=f"{label} input '{name}'",
            )
    ...
    *,
    ...
    if exact:
        ...
        if name not in inputs:
            raise _fail(
                "SDAI-WFCOMP-004",
                f"{label} references unresolved input '{name}'",
            )
    ...
    if "${{" in rendered or "}}" in rendered:
        raise _fail(
            "SDAI-WFCOMP-004",
            f"{label} contains malformed or unsupported input expression",
        )
    return rendered
    ...
    *,
    ...
    if isinstance(raw, str):
        if not _SAFE_NAME.fullmatch(raw.strip()):
            raise _fail(
                "SDAI-WFCOMP-005",
                f"{label} shorthand step id is invalid",
            )
        return
    if not isinstance(raw, Mapping):
    ...
    if allowed is None:
        raise _fail(
            "SDAI-WFCOMP-005",
            f"{label} has unsupported step type '{kind}'",
        )
    ...
    if kind == "parallel":
        ...
        if not isinstance(children, list) or not children:
            raise _fail(
                "SDAI-WFCOMP-005",
                f"{label} parallel steps must be a non-empty list",
            )
...
def _component_path(project_root: Path, component_id: str) -> Path:
    root = project_root.resolve()
    candidates = (
        ensure_within_project(
            root,
            root / ".sdai" / "workflow-components" / f"{component_id}.yaml",
            label=f"workflow component '{component_id}' canonical path",
        ),
        ensure_within_project(
            root,
            root
            ...
            / "workflow-components"
            / f"{component_id}.yaml",
            label=f"workflow component '{component_id}' compatibility path",
        ),
    )
    existing = [path for path in candidates if path.exists()]
    if len(existing) > 1:
        locations = ", ".join(_portable(root, path) for path in existing)
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' exists in more than one location: {locations}",
        )
    if not existing:
        expected = ", ".join(_portable(root, path) for path in candidates)
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' does not exist; searched: {expected}",
        )
    path = existing[0]
...
def _load_component(
    ...
    if not _COMPONENT_REF.fullmatch(f"component:{component_id}"):
        raise _fail(
            "SDAI-WFCOMP-001",
            f"invalid workflow component id '{component_id}'",
        )
    root = project_root.resolve()
    path = _component_path(root, component_id)
    try:
        raw = yaml.safe_load(read_utf8_text(path)) or {}
    except (OSError, TextEncodingError, yaml.YAMLError) as exc:
        raise _fail(
            "SDAI-WFCOMP-001",
            f"unable to read workflow component '{component_id}': {exc}",
        ) from exc
    if not isinstance(raw, Mapping):
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' must be a YAML mapping",
        )
    try:
        ...
        raise _fail(
            "SDAI-WFCOMP-001",
            f"invalid workflow component '{component_id}': {exc}",
        ) from exc
    ...
    if not isinstance(raw_requires, list) or not all(
        ...
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' requires must be a string list",
        )
    requires = tuple(item.strip() for item in raw_requires)
    if any(
        not _COMPONENT_REF.fullmatch(f"component:{item}")
        for item in requires
    ):
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' contains invalid required component id",
        )
    if len(requires) != len(set(requires)) or component_id in requires:
        raise _fail(
            "SDAI-WFCOMP-006",
            f"workflow component '{component_id}' has duplicate/self dependency",
        )
    raw_steps = manifest.spec.get("steps")
    if not isinstance(raw_steps, list) or not raw_steps:
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow component '{component_id}' steps must be a non-empty list",
        )
...
def _step_ids(raw_steps: list[object]) -> tuple[str, ...]:
    ...
    for raw in raw_steps:
        ...
        if isinstance(raw, Mapping):
            ...
            children = (
                raw.get("steps")
                if str(raw.get("type") or raw.get("kind") or "") == "parallel"
                else None
            )
...
def _validate_component_dependencies(
    uses: tuple[ComponentUseProvenance, ...],
) -> None:
    ...
    def visit(component_id: str) -> None:
        ...
        if component_id in visiting:
            ...
            raise _fail(
                "SDAI-WFCOMP-006",
                "workflow component dependency cycle: "
                + " -> ".join(cycle),
            )
...
def _has_component_use(raw_steps: object) -> bool:
    return isinstance(raw_steps, list) and any(
        isinstance(item, Mapping) and "uses" in item
...
def compose_workflow(
    ...
    *,
    ...
    Existing workflows with neither typed-input features nor component uses are
    returned byte-structurally unchanged and are never interpreted for `${{ ... }}`
    expressions. Component expansion happens before the existing workflow step
    parser, so all current step-kind safety rules remain the final validator.
    """
    ...
    if not isinstance(raw_steps, list):
        ...
        raise _fail(
            "SDAI-WFCOMP-001",
            f"workflow '{workflow_name}' steps must be a list",
        )
    uses_components = _has_component_use(raw_steps)
    uses_inputs = (
        "inputs" in workflow_data
        or "input_values" in workflow_data
        or bool(input_values)
    )
    if not uses_components and not uses_inputs:
    ...
    workflow_name = str(workflow_data.get("name") or "workflow")
    workflow_inputs = _parse_inputs(
        workflow_data.get("inputs"),
    ...
    if not isinstance(declared_values, Mapping):
        raise _fail(
            "SDAI-WFCOMP-002",
            f"workflow '{workflow_name}' input_values must be a mapping",
        )
    ...
    for index, raw_step in enumerate(raw_steps):
        ...
        if (
            not isinstance(workflow_resolved, Mapping)
            or "uses" not in workflow_resolved
        ):
        ...
        if not isinstance(raw_ref, str):
            raise _fail(
                "SDAI-WFCOMP-001",
                f"workflow '{workflow_name}' component use #{index + 1} uses must be a string",
            )
    ...
    return WorkflowComposition(
        ...
        resolved_workflow_inputs=_redacted_inputs(
            workflow_inputs,
            resolved_workflow_inputs,
        ),
